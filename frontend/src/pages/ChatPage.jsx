...
const ChatPage = () => {
    ...
    const loadChats = async () => {
      try {
        ...
        if (data.chats.length) {
          ...
          setSelectedCourseCode(firstChat.course_code || '');
        }
      } catch (err) {
        if (isMounted) {
          setError(err.message);
  ...
  const handleCreateChat = async () => {
    ...
    try {
      const data = await apiRequest('/chats', {
        method: 'POST',
        body: { title: 'New Chat', courseCode: selectedCourseCode || undefined },
      });
      setChats((prev) => [data.chat, ...prev]);
      setSelectedChatId(data.chat.id);
      setSelectedCourseCode(data.chat.course_code || '');
      setMessages([]);
  ...
  const handleSelectChat = (chatId) => {
    ...
    setSelectedCourseCode(chat?.course_code || '');
    setSidebarOpen(false);
    setShowMenu(false);
    setError('');
  };
  ...
  const activeCourseCode = selectedChat?.course_code || '';
  ...
  const markdownComponents = {
    code({ node, inline, className, children, ...props }) {
      return inline ? (
        ...
        <pre className="bg-gray-900 text-gray-100 text-xs rounded p-3 overflow-x-auto" {...props}>
    ...
    <div className="flex h-screen bg-gray-100 dark:bg-gray-900">
      ...
      <div className="flex-1 flex flex-col ml-0 md:ml-64">
        ...
        <div className="flex-1 overflow-y-auto flex flex-col px-6 pt-6 pb-28">
          ...
          <div className="mb-4 grid gap-3 md:grid-cols-2 items-start">
            <div>
              <label className="block text-xs text-gray-500 mb-1">Curso / RAG</label>
              <select
                value={selectedCourseCode}
                onChange={(e) => setSelectedCourseCode(e.target.value)}
                className="w-full md:max-w-xs border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900 text-sm rounded px-3 py-2"
                disabled={loadingCourses}
              >
                <option value="">Chat default (sin RAG)</option>
                {courses.map((course) => (
              ...
              <p className="text-xs text-gray-500 mt-1">
                Selecciona un curso y luego crea un chat para usar sus documentos. Los chats existentes muestran el curso fijo que
                tienen asignado.
              </p>
            </div>
            {activeCourseCode ? (
              <div className="flex items-center gap-2 text-sm bg-blue-50 border border-blue-100 text-blue-800 px-3 py-2 rounded">
                <span className="font-semibold">Chat actual:</span> {activeCourseCode}
              </div>
            ) : (
              <div className="flex items-center gap-2 text-sm text-gray-600">
                {loadingCourses ? 'Cargando cursos...' : 'El chat actual usa el modelo general.'}
              </div>
            )}
          </div>
          {loadingMessages ? (
