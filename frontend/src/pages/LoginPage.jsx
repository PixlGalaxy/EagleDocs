...
function LoginPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      navigate(loggedUser.role === 'instructor' ? '/instructor' : '/chat');
    } catch (err) {
    ...
    <div className="h-screen flex items-center justify-center bg-gray-50">
      <form onSubmit={handleSubmit} className="w-full max-w-md bg-white rounded-lg shadow-md p-6 space-y-4">
        ...
        <div className="rounded-md bg-blue-50 border border-blue-100 p-3 text-sm text-blue-800">
          Signing in as a {detectedRole}. Emails ending in <span className="font-semibold">@fgcu.edu</span> are treated as
          instructors and will be redirected to the instructor workspace.
        </div>
        <div>
